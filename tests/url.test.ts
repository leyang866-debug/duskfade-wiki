...
describe('url helpers', () => {
  describe('localizePath', () => {
    ...
    it('prepends the locale prefix for non-default locales', () => {
      expect(localizePath('/weapons', 'zh')).toBe('/zh/weapons');
  ...
  describe('listPath', () => {
    it('builds the correct list URL for each locale', () => {
      ...
      expect(listPath('guides', 'zh')).toBe('/zh/guides');
  ...
  describe('detailPath', () => {
    ...
    it('maps the internal 1.0 guide slug to its public URL', () => {
      ...
      expect(detailPath('guides', '10-update', 'zh')).toBe('/zh/1.0-update');
    });
  });

  describe('localeFromPath', () => {
    it('extracts the locale from a prefixed path', () => {
      expect(localeFromPath('/zh/weapons')).toBe('zh');

...
async function getWikiEntries(): Promise<WikiEntry[]> {
  try {
...
export async function getEntryWithFallback(
  ...
  const requested = entries.find((entry) => entry.id.replace(/\.mdx$/, '') === id);
  if (requested) return { entry: requested, servedLocale: locale, isFallback: false };

  if (locale === defaultLocale) return null;
  const fallbackId = `${defaultLocale}/${category}/${slug}`;
  const fallback = entries.find((entry) => entry.id.replace(/\.mdx$/, '') === fallbackId);
  return fallback ? { entry: fallback, servedLocale: defaultLocale, isFallback: true } : null;
}
